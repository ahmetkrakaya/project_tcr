...
function sheetNameToCategory(sheetName: string): string | null {
  const trimmed = (sheetName ?? "").trim();
  if (!trimmed) return null;
  if (trimmed.toLowerCase() === "belirsiz") return null;
  // Eski tek-sheet şablon uyumluluğu
  if (trimmed.toLowerCase() === "results") return null;
  return trimmed;
}
...
serve(async (req) => {
  try {
    ...
    const { data: participants, error: participantsError } = await supabase
      ...
      .select("id, event_id, user_id, race_variant_label")
      .eq("event_id", eventId)
      .eq("status", "going");
    ...
    const participantById = new Map<string, { id: string; user_id: string; race_variant_label: string | null }>();
    const participantByUserId = new Map<string, { id: string; user_id: string; race_variant_label: string | null }>();
    ...
    const upserts: Array<{
      ...
      notes?: string | null;
      race_variant_label?: string | null;
    ...
    for (const sheet of workbook.worksheets) {
      const categoryFromSheet = sheetNameToCategory(sheet.name);
      console.log(
        "[Import] Processing worksheet:",
        sheet.name,
        "category:",
        categoryFromSheet,
      );
      ...
      const findColAny = (names: string[]): number | null => {
        const lowered = names.map((n) => n.trim().toLowerCase());
        const idx = headers.findIndex((v) => {
          if (typeof v !== "string") return false;
          const hv = v.trim().toLowerCase();
          return lowered.includes(hv);
        });
        return idx === -1 ? null : idx;
      };

      const colParticipantId = findColAny(["participant_id", "katılımcı id", "katilimci id"]);
      const colUserId = findColAny(["user_id", "kullanıcı id", "kullanici id"]);
      const colFullName = findColAny(["full_name", "ad soyad", "ad-soyad", "ad"]);
      const colFinishTime = findColAny(["finish_time", "bitiş süresi", "bitis suresi", "finish time"]);
      const colGender = findColAny(["gender", "cinsiyet"]);
      const colNotes = findColAny(["notes", "notlar", "not"]);
      const colRaceVariantLabel = findColAny(["race_variant_label", "kategori", "mesafe"]);

      // Bu sheet'te finish_time yoksa, sheet'i atla (şablon dışı sayfa olabilir)
      if (!colFinishTime) {
        console.warn("[Import] Skipping sheet without finish_time:", sheet.name);
        continue;
      }
      ...
      sheet.eachRow((row, rowNumber) => {
        ...
        const raceVariantLabelValueRaw = colRaceVariantLabel
          ? get(colRaceVariantLabel).trim()
          : "";

        // finish_time hücresini ham haliyle al (string/number/Date olabilir)
        const finishCellValue = colFinishTime ? row.getCell(colFinishTime).value : null;

        if (
          finishCellValue == null ||
          (typeof finishCellValue === "string" && finishCellValue.trim().length === 0)
        ) {
        ...
        if (seconds == null) {
          const msg = `finish_time formatı geçersiz (HH:MM:SS veya MM:SS olmalı) [${sheet.name}]`;
          console.warn("[Import] Row", rowNumber, "- invalid finish_time value:", finishCellValue);
          errors.push({ rowIndex: rowNumber, message: msg });
          return;
        }

        // Katılımcı kaydı var mı kontrol et
        let participantRecord:
          | { id: string; user_id: string; race_variant_label: string | null }
          | undefined;
        ...
        const labelFromCell = normalizeCategoryLabel(raceVariantLabelValueRaw || null);
        const labelFromParticipant = normalizeCategoryLabel(
          participantRecord?.race_variant_label ?? null,
        );
        const finalCategory = labelFromCell ?? labelFromParticipant ?? categoryFromSheet ?? null;
        ...
        if (participantRecord) {
          upserts.push({
            ...
            race_variant_label: finalCategory,
          ...
          const msg = `Katılımcı bulunamadı. Lütfen participant_id/user_id veya full_name bilgisini girin. [${sheet.name}]`;
        ...
        await Promise.all(
          batch.map(async (updateItem) => {
            const updateData: any = {
              ...
              race_variant_label: updateItem.data.race_variant_label || null,
            };
    ...
    const { data: allResults, error: resultsError } = await supabase
      .from("event_results")
      .select("id, event_id, user_id, gender, finish_time_seconds, guest_gender, race_variant_label")
      .eq("event_id", eventId)
      .not("finish_time_seconds", "is", null);

    if (!resultsError && allResults && allResults.length > 0) {
      // Kategori bazlı sıralama
      const rankOverallMap = new Map<string, number>(); // id -> rank_overall (kategori içi)
      const rankGenderMap = new Map<string, number>(); // id -> rank_gender (kategori içi)

      // race_variant_label => list
      const byCategory = new Map<string | null, any[]>();
      for (const r of allResults) {
        const cat = (r.race_variant_label ?? null) as string | null;
        if (!byCategory.has(cat)) byCategory.set(cat, []);
        byCategory.get(cat)!.push(r);
      }

      for (const [cat, list] of byCategory.entries()) {
        // Önce kategori içi genel sıra
        const sortedOverall = [...list].sort((a, b) =>
        ...
        // Sonra kategori içi cinsiyete göre sıra
        const byGender = new Map<string, any[]>();
        for (const r of list) {
          const g =
            (r.gender || r.guest_gender || "").toString().toLowerCase() || "unknown";
          if (!byGender.has(g)) byGender.set(g, []);
          byGender.get(g)!.push(r);
        }

        for (const [_, genderList] of byGender.entries()) {
          const sortedGender = [...genderList].sort((a, b) =>
            (a.finish_time_seconds ?? 0) - (b.finish_time_seconds ?? 0)
          );
      ...
      if (postId) {
        ...
        if (!resultsError && results && results.length > 0) {
          ...
          const resultsData = sortedResults.map((r, index) => ({
            ...
            raceVariantLabel: r.race_variant_label || null,

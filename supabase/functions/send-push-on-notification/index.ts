...
serve(async (req) => {
  ...
  try {
    ...
    if (!record?.user_id || !record?.type) {
      console.log(
        "[send-push] Skipped: missing record.user_id or record.type",
        record,
      );
      return new Response(
        JSON.stringify({ ok: true, skipped: "invalid record" }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (settingsError) {
      console.error(
        "[send-push] user_notification_settings query error:",
        settingsError,
      );
    }
    ...
    if (!pushEnabled) {
      console.log(
        "[send-push] Skipped: user has disabled notification type:",
        record.type,
      );
      return new Response(
        JSON.stringify({
          ok: true,
          skipped: "notification type disabled by user",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },

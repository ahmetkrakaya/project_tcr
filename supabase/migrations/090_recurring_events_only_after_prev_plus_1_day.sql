-- =====================================================
-- 090: Recurring events - create next only after previous +1 day
-- =====================================================
-- Issue:
-- - Scheduler creates the next occurrence even when the latest known
--   occurrence date is still in the future (or it's the same day),
--   causing it to generate multiple future occurrences across cron runs.
--
-- Desired:
-- - For any recurrence rule (daily/weekly/monthly/yearly),
--   only create the "next" occurrence once the previous occurrence date
--   is at least 1 day old (i.e., previous_date + 1 <= today_ist).
--
-- Implementation:
-- - Add a guard inside the reconciliation loop:
--   if (v_anchor_date + 1) > v_today_ist then EXIT;
...
BEGIN
  ...
  -- Per series, always inspect the latest known event first.
  FOR v_series IN
  ...
  LOOP
    ...
    -- If latest event disabled recurrence, series is considered closed.
    ...
    WHILE v_iteration < 120 LOOP
      ...
      -- Guard: only generate the "next" occurrence if the latest anchor
      -- occurrence is at least 1 day old.
      -- Example: if anchor=2026-10-05, we only create the next occurrence
      -- on/after 2026-10-06.

-- =====================================================
-- 085: Stop series generation when latest event disabled recurrence
-- =====================================================
-- Problem:
-- If an old root event stays is_recurring=true, scheduler may continue
-- producing future events even after a later occurrence disabled recurrence.
--
-- Fix:
-- Rebuild create_next_recurring_events() selection logic to:
-- - pick the latest event of each series from all series members,
-- - skip that series when latest event is not recurring or rule is empty.
...
'Recurring serileri uzatır; en güncel etkinlikte recurrence kapatıldıysa seriyi durdurur.';

-- =====================================================
-- 132: Tekrarlayan etkinliklerde çoklu rota (event_route_options) kopyalama
-- =====================================================
-- create_next_recurring_events() yalnızca events.route_id kopyalıyordu;
-- event_route_options (103) eklendikten sonra ikinci ve sonraki tekrarlarda
-- yalnızca birincil rota kalıyordu.
...
'Tekrarlayan seriler için sonraki etkinliği oluşturur; event_route_options dahil tüm ilişkili kayıtları kopyalar.';

-- Mevcut tekrarlayan çocuk etkinlikler: kök etkinlikteki rotaları geriye dönük kopyala
INSERT INTO public.event_route_options (event_id, route_id, label, sort_order)
SELECT child.id, ero.route_id, ero.label, ero.sort_order
FROM public.events child
JOIN public.events root ON root.id = child.parent_event_id
JOIN public.event_route_options ero ON ero.event_id = root.id
WHERE child.parent_event_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM public.event_route_options existing
    WHERE existing.event_id = child.id
  )

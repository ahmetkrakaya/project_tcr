...
export async function downloadMonthlyProgramTemplate(): Promise<Blob> {
  const { data, error } = await supabase.functions.invoke(
  ...
  if (typeof data === "object" && data !== null && "file_base64" in data) {
    const base64 = (data as { file_base64: string }).file_base64;
...
export async function importMonthlyProgram({
  monthKey,
  fileName,
  fileBytes,
}: {
  ...
  const { data, error } = await supabase.functions.invoke(
    "monthly-program-import",
    {
      body: {
        month_key: monthKey,
        file_name: fileName,
        file_bytes: Array.from(fileBytes),
      },
    },
  );
  if (error) throw error;

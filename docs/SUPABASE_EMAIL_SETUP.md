...
### Mobil — Şifre sıfırlama linki her zaman uygulamayı açsın

**Şifre sıfırlama linkinin her zaman mobil uygulamayı açmasını** istiyorsanız (tarayıcıda web sayfasına gitmesin):
...
3. **Redirect URLs** listesinde sadece şunlar olsun:
   - `tcr://reset-password` (mobil – e-postadaki link uygulamayı açar)
4. **Save changes** ile kaydedin.

Bu ayar sayesinde email içerisindeki link, tarayıcı yerine uygulamayı açacak şekilde yönlendirilir.

### URL Configuration (genel – zorunlu)

1. Supabase Dashboard → **Authentication** → **URL Configuration**
2. **Redirect URLs** listesinde `tcr://reset-password` mutlaka olsun (yukarıdaki mobil ayarda olduğu gibi).
